var initspatiasql = function (cb) {

	var spatialite_version = Module['cwrap']('spatialite_version', 'string', ['number']),
	...
	**Warning**: When you close a database (using db.close()), all its statements are
	...
	for (i in this['spatiasql']) {
		Module[i] = this['spatiasql'][i];
	}

	/* wasm requires cb */
	return cb ? cb(this['spatiasql']) : this['spatiasql'];
