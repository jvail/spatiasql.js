...
var testcases = fs.readdirSync('sql_stmt_geopackage_tests');
for (var i = 0, is = testcases.length; i < is; i++) {

  if (testcases[i].indexOf('.testcase') < 0 || !fs.lstatSync('sql_stmt_geopackage_tests/' + testcases[i]).isFile())
    continue;
  var testcase = fs.readFileSync('sql_stmt_geopackage_tests/' + testcases[i], { encoding: 'utf-8' });
