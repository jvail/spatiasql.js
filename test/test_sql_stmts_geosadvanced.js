...
var testcases = fs.readdirSync('sql_stmt_geosadvanced_tests');
for (var i = 0, is = testcases.length; i < is; i++) {

  if (testcases[i].indexOf('.testcase') < 0 || !fs.lstatSync('sql_stmt_geosadvanced_tests/' + testcases[i]).isFile())
    continue;
  var testcase = fs.readFileSync('sql_stmt_geosadvanced_tests/' + testcases[i], { encoding: 'utf-8' });
  if (testcase.length > 0) {
    ...
    var stmt = lines[2];
    // it seems CreateTopology test failed initially because it uses a new database with no spatial meta data
    // we create the meta data
    if (stmt.indexOf('CreateTopology') > -1)
